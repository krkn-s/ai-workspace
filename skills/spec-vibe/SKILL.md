---
name: spec-vibe
description: Run a lightweight, CLI-free, spec-driven development workflow over a pure-Markdown specs/ tree — propose changes, write behavior-first specs and deltas, design, tasks, implement, then archive by merging deltas into the current source of truth. Supports the forward path (spec first, then code) and a reverse path (vibe-code first, then retroactively generate and validate specs). Use whenever the agent is about to build, modify, or refactor a feature, behavior, API, schema, or contract; when code is being changed without an associated spec; when the user mentions specs, proposals, deltas, ADRs, change folders, acceptance criteria, living specs, or a spec-driven / vibe-coding workflow; or when auditing a repo for spec drift (code with no spec). Déclenche aussi sur les requêtes en françai, par exemple créer ou rédiger une spec, une proposition, un delta ou un ADR, formaliser une fonctionnalité, générer des specs à partir de code déjà écrit, auditer la dérive specs vs code, mettre en place un workflow spec-driven ou vibe coding sans CLI ni base de données, organiser une arborescence specs avec changes classés par états.
---
