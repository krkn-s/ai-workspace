...
- **prompts**
  ...
  - [`agents-md-multiagent`](prompts/agents-md-multiagent.md) : Ensure a project's shared `AGENTS.md` contains the multi-agent coordination section (worktree-first, `.agents-sync/` with `state.json` + `messages.jsonl`, session protocol, merge rules) — idempotent append when missing, no-op when present.
