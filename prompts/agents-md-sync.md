...
- Walk parent directories from the project root upward for `AGENTS.md` or `AGENTS.override.md` files — pi concatenates them into the effective context.
- Read the project file — `AGENTS.override.md` if present, else `AGENTS.md`. If neither exists, propose creating a lean one grounded in the repo and the cascade, then stop.
...
- **Duplicate** — restates a global rule verbatim or near-verbatim. Remove it — pi already loads the cascade, repetition only costs tokens — unless the file must stand alone for other tools; ask when unclear.
...
- Re-check the applied file against the cascade — no contradiction, no duplication, explicit overrides only.
